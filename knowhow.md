...
### Bước 2.6: Khởi tạo cơ sở dữ liệu (Database Schema)
Hãy đọc tệp [schema.sql](file:///backend/src/db/schema.sql) và sao chép toàn bộ nội dung để chạy trong SQL Editor của dự án Supabase nhằm thiết lập:
- Bảng `profiles`, `lessons`, `vocabulary`, `kanji`, `grammar`, `user_progress`, `target_plans`.
- Trigger tự động đồng bộ tài khoản auth Supabase sang bảng `profiles`.
- Các chính sách bảo mật Row Level Security (RLS) phân quyền chi tiết cho User và Admin.

---

### Bước 2.7: Kiểm tra xác minh môi trường (Verification)
Hãy chạy các kiểm thử tích hợp (integration tests) để xác minh cơ chế phân quyền (middleware) hoạt động chính xác:
```bash
npm run test:backend
```
AI Agent cần kiểm tra log đầu ra để xác nhận các bài kiểm thử đạt kết quả thành công (`All integration routing tests passed successfully!`).

Ngoài ra, có thể khởi động máy chủ nhà phát triển để kiểm tra chạy đồng thời:
```bash
npm run dev
```
**AI Agent cần kiểm tra các log đầu ra để xác nhận:**
- Frontend khởi chạy thành công trên cổng `3000`.
- Backend Express khởi chạy thành công trên cổng `8080`.

Sau khi xác minh xong, hãy tắt (kill) tác vụ để giải phóng cổng cho người dùng.
...
4. **Thiết kế Responsive**: Mọi giao diện viết ở Frontend phải hỗ trợ hiển thị tối ưu trên thiết bị di động (Mobile-First responsive sử dụng Tailwind CSS).
5. **Bảo mật API**: 
...
7. **Khởi động lại Backend khi sửa đổi**: Do backend Express chạy bằng Node thường (`node src/index.js`) mà không có nodemon, bất kỳ chỉnh sửa nào trong backend (bao gồm cả dữ liệu giả lập `mockDb.js`) sẽ không tự động cập nhật. AI Agent cần tắt server cũ và khởi chạy lại `npm run dev` để kiểm tra thay đổi backend.
8. 🛡️ **Bảo toàn dữ liệu học tập**: Tuyệt đối KHÔNG được thực hiện các thay đổi cấu trúc database, reindex ID học liệu, hoặc chạy các kịch bản seed/cập nhật database trực tiếp làm ảnh hưởng, xáo trộn hoặc xóa mất tiến trình học tập (`user_progress`, `target_plans`) và thông tin tài khoản (`profiles`) của người dùng trên môi trường production/online.
